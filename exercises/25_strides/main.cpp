...
std::vector<udim> strides(std::vector<udim> const &shape) {
    ...
    // READ: 逆向迭代器 std::vector::rbegin 
    auto shape_rbegin = shape.rbegin();
    auto shape_rend = shape.rend();
    auto strides_it = strides.rbegin();
    ++strides_it;
    for (auto it = shape_rbegin; it != shape_rend - 1; ++it) {
        *strides_it = (*(strides_it - 1)) * (*it);
        ++strides_it;
    }
